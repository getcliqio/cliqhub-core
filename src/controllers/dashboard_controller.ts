/**
 * Dashboard Hub resource — DASH-ORG invent + DASH-S0 MVC structure.
 *
 * POST /internal/dashboard/realms | summary: body `org_id` required.
 * Never invent org from X-Org-Id / current_org_id.
 * Envelope stays flat until a future DASH-ENV slice.
 */
...
} from '../schemas/dashboard/inputs.js';
...
export class DashboardController extends BaseController {
     ...
     *
     * @param req - Body: {@link DashboardRealmsInput}
     * @param res - Flat `{ ok: true, realms, totals }`
     */
    async realms_summary(
     ...
     * Body `org_id` required (DASH-ORG hard-cut).
     *
     * @param req - Body: {@link DashboardSummaryInput}
     * @param res - Flat `{ ok: true, counts, … }`
     */
    async summary(
