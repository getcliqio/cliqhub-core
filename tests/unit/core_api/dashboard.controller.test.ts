...
describe('DashboardController.summary', () => {
    ...
    it('401 when caller has no user_id', async () => {
        ...
        await dashboard.summary(req as never, res as never);

        expect(res.status_code).toBe(401);
        expect(DaemonService.list).not.toHaveBeenCalled();
    });

    it('without org_id → 422; no invent', async () => {
        const req = make_req({});
        const res = mock_res();
        await expect(dashboard.summary(req as never, res as never)).rejects.toMatchObject({
            status: 422,
            code: 'invalid_params',
        });
        expect(DaemonService.list).not.toHaveBeenCalled();
